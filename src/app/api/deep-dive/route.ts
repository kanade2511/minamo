...
export async function POST(request: NextRequest) {
    ...
    if (!noteContent) {
        return new Response('Missing noteContent', { status: 400 })
    }
  ...
  const allMessages = [systemMsg, noteMsg, ...history]
    ...
    try {
        ...
        return new Response('LLM APIの設定が正しくありません。環境変数を確認してください。', {
            status: 500,
        })
    }
}
