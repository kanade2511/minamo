...
export default function ExploreDetail({ entry }: { entry: Entry }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
      <div className="flex gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-4 flex justify-end">
            <DeleteEntryButton entryId={entry.id} redirectTo="/app/explore" />
          </div>

          {/* Analysis */}
          <div className="mt-10 lg:hidden">
            <div className="rounded-lg border border-border p-4">
              <h2 className="text-[10px] text-text-secondary/40 tracking-wider mb-4">エントリ分析</h2>
              <EntryAnalysis content={entry.content} />
            </div>
          </div>
        </div>
      ...
      <div className="mt-8 lg:hidden">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-1.5 h-1.5 rounded-full bg-accent/40" />
          <h2 className="text-xs text-text-secondary tracking-wider font-medium">Mirror との対話</h2>
        </div>
        <InlineConversation
          entryId={entry.id}
          entryContent={entry.content}
