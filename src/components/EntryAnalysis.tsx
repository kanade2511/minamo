...
export default function EntryAnalysis({ content }: { content: string }) {
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[10px] text-text-secondary/40 tracking-wider">感情の傾向</span>
          <span className="text-[10px] text-text-secondary/30" style={{ color: sentimentColor }}>
            {sentimentLabel}
          </span>
        </div>
      </div>
      ...
      {result.emotions.length > 0 && (
        <div className="space-y-2">
          <p className="text-[10px] text-text-secondary/40 tracking-wider">検出された感情</p>
          {result.emotions
            .filter((em) => em.score > 0.05)
            .sort((a, b) => b.score - a.score)
            .map((em) => (
              <div key={em.label} className="flex items-center gap-2">
                <span className="text-[10px] text-text-secondary/50 w-8 text-right truncate">{em.label}</span>
      ...
      {result.keywords.length > 0 && (
        <div>
          <p className="text-[10px] text-text-secondary/40 tracking-wider mb-2">キーワード</p>
      ...
      {result.summary && (
        <div className="pt-2 border-t border-border">
          ...
          <p className="text-xs text-text-secondary/60 leading-relaxed">{result.summary}</p>
        </div>
      )}
      ...
      {loading && (
        <div className="flex items-center gap-1.5 pt-1">
          ...
          <span className="w-1 h-1 rounded-full bg-accent/30 animate-pulse" style={{ animationDelay: "300ms" }} />
          <span className="w-1 h-1 rounded-full bg-accent/30 animate-pulse" style={{ animationDelay: "600ms" }} />
        </div>
      )}
    </div>
  );
