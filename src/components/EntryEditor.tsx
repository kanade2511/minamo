...
export default function EntryEditor({ question, userId }: Props) {
          ...
          {savedEntry && (
            <div className="mt-6 p-4 rounded-lg border border-accent/20 bg-accent/5">
              <p className="text-xs text-text-secondary/60 leading-relaxed">
                保存したエントリは <span className="text-accent">「探る」</span> タブからMirrorと対話できます
              </p>
            </div>
          )}
        </div>

        {/* Right: Entry Analysis */}
        <div className="w-80 flex-shrink-0 hidden lg:block">
          <div className="sticky top-20">
            <div className="rounded-lg border border-border p-4">
              <h2 className="text-[10px] text-text-secondary/40 tracking-wider mb-4">エントリ分析</h2>
              <EntryAnalysis content={content} />
            </div>
          </div>
        </div>
      </div>
