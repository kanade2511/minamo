...
export default function InlineConversation({
    ...
    const sendMessage = async () => {
        ...
        try {
            const res = await fetch('/api/deep-dive', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ noteContent, messages: updatedMessages }),
            })
            if (!res.ok) throw new Error('API error')
            const reader = res.body?.getReader()
            if (!reader) return
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await saveInsight(noteId, messages, lastAssistantMsg.content, [])
            setSaved(true)
        } catch (e) {
    ...
    return (
        <div className='rounded-lg border border-border overflow-hidden'>
                ...
                {messages.length === 0 && isStreaming && (
                    <div className='flex flex-col items-center justify-center py-10 gap-3'>
                        <div className='flex gap-1.5'>
                            <span
                                className='w-2 h-2 rounded-full bg-accent/20 animate-bounce'
                ...
                {messages.map((msg, i) => (
                    ...
                    >
                        <div className='max-w-[80%]'>
                            <p
                                className={`text-[10px] text-accent/50 mb-1 tracking-wider ${msg.role === 'assistant' ? 'text-left' : 'text-right'}`}
                            >
            ...
            <div className='px-5 py-4 border-t border-border bg-surface-card'>
                ...
                <p className='mt-1.5 text-[10px] text-text-secondary/25 text-right'>
                    Cmd+Enter で送信
                </p>
            </div>
        </div>
